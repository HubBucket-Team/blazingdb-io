...
namespace Library {
    namespace Logging {
        TcpOutput::TcpOutput()
        : isActive{false}, isReady{false}, isDataReadyToSend{false} {
            srand(time(NULL));
            nodeInd = (unsigned int)((rand() + 1) * 1000); // times 1000 so that it is obvious that its not an actual nodeInd but a temporary unique id
        }
        ...
        void TcpOutput::setWaitTime(std::chrono::milliseconds&& value) {
            waitTime = value;
        }

        void TcpOutput::setWaitTime(const std::chrono::milliseconds& value) {
            waitTime = value;
        }

        void TcpOutput::setSocket(std::shared_ptr<Library::Network::GenericSocket>& value) {
            socket = value;
        }
        ...
        void TcpOutput::flush(std::string&& log) {
            doOnProducer(log);
        }

        void TcpOutput::flush(const std::string& log) {
            doOnProducer(log);
        }

        void TcpOutput::setNodeIdentifier(const unsigned int nodeInd) {

        	std::string message = "Node index " + std::to_string(nodeInd) + " was using temporary node index identifier " + std::to_string(this->nodeInd);
