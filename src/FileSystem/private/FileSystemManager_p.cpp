...
bool FileSystemManager::Private::registerFileSystem(const FileSystemEntity &fileSystemEntity) {
	...
	for (int i = 0; i < this->fileSystems.size(); ++i) {
		const bool found = (this->fileSystems[i]->getFileSystemConnection() == fileSystemConnection) && this->fileSystems[i]->getRoot() == root;
...
bool FileSystemManager::Private::deregisterFileSystem(const std::string& authority) {
	...
	if (deleteFileSystem) {
		auto fileSystem = std::move(this->fileSystems[fileSystemId]); // transfer ownership so it can be deleted within this scope
...
std::vector<FileStatus> FileSystemManager::Private::list(const Uri &uri, FileType fileType, const std::string &wildcard) const {
	if (uri.isValid() == false) {
		//TODO percy thrown exception
	}
	try {
...
std::vector<std::string> FileSystemManager::Private::listResourceNames(const Uri &uri, FileType fileType, const std::string &wildcard) const {
	if (uri.isValid() == false) {
		//TODO percy thrown exception
	}
	try {
...
std::vector<std::string> FileSystemManager::Private::listResourceNames(const Uri &uri, const std::string &wildcard) const {
	if (uri.isValid() == false) {
		//TODO percy thrown exception
	}
	try {
...
bool FileSystemManager::Private::move(const Uri& src, const Uri& dst) const {
	...
	try {
		...
		if (fileSystemIdSrc != fileSystemIdDst) {
			...
			// TODO when we implement the copy operation in the FileSystemManager, we can replace the manual copy step and replace with a general copy
