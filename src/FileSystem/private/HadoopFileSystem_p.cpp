...
bool HadoopFileSystem::Private::connect(const FileSystemConnection &fileSystemConnection) {
	...
	//TODO we currently only support LIBHDFS3
	//if(connectionProperties.at(HadoopFileSystem::ConnectionProperties.at(HadoopFileSystem::ConnectionProperty::DRIVER)) == HadoopFileSystem::DriverTypes.at(HadoopFileSystem::DriverType::LIBHDFS)){
...
FileStatus HadoopFileSystem::Private::getFileStatus(const Uri &uri) const {
	...
	if (result.ok()) {
		...
		switch (stat_buf.kind) {
			case arrow::io::ObjectType::type::FILE:
				fileType = FileType::FILE;
				break;

			case arrow::io::ObjectType::type::DIRECTORY:
...
std::vector<Uri> HadoopFileSystem::Private::list(const Uri &uri, const std::string &wildcard) const {
	...
	if (status.ok()) {
		...
		if (this->root.isRoot()) { // if root is '/' then we don't need to replace the uris to relative paths
			...
			for (auto hdfsPathInfo : listing) {
				const bool pass = WildcardFilter::match(hdfsPathInfo.name, finalWildcard); //filter must use the full path

				if (pass) {
					Path relativePath(hdfsPathInfo.name);
...
std::vector<std::string> HadoopFileSystem::Private::listResourceNames(const Uri &uri, FileType fileType, const std::string &wildcard) const {
	...
	if (status.ok()) {
		...
		if (this->root.isRoot()) { // if root is '/' then we don't need to replace the uris to relative paths
			...
			for (auto hdfsPathInfo : listing) {
				...
				if (pass) {
					response.push_back(fullPath.getResourceName()); // resource name is the same for full paths or relative paths
				}
			}
		}
	} else {
...
std::vector<std::string> HadoopFileSystem::Private::listResourceNames(const Uri &uri, const std::string &wildcard) const {
	std::vector<std::string> response;
