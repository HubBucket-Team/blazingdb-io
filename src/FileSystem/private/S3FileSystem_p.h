...
class S3FileSystem::Private {
	...
	private:
		...
		const S3FileSystemConnection::EncryptionType encryptionType() const; // get the encryption type from the current s3 file system connection
		bool isEncrypted() const; // returns true if the connection is encrypted (AES-256 or AWS-KMS), returns false when EncryptionType is None
		const Aws::S3::Model::ServerSideEncryption serverSideEncryption() const; // get the encryption type from the current s3 file system connection
		bool isAWSKMSEncrypted() const; // returns true if the connection is encrypted with AWS-KMS
		const std::string getSSEKMSKeyId() const; // if isAWSKMSEncrypted is true then returns the KMS_KEY_AMAZON_RESOURCE_NAME
