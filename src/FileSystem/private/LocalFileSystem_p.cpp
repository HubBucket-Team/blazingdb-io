...
LocalFileSystem::Private::Private(const Path &root)
	: root(root) {
}
...
inline void openDirExceptions(Uri uri) {
	switch (errno) {
		case EACCES:
			throw BlazingInvalidPermissionsFileException(uri);
		case EBADF:
			throw BlazingFileSystemException("The file descriptor to " + uri.toString() + " is no longer valid.");
		case ENOENT:
			throw BlazingFileNotFoundException("File at " + uri.toString() + " does not exist.");
		case EMFILE:
			throw BlazingFileSystemException("The per process limit of files has been reached. Consider increasing the per process limit using ulimit");
		case ENFILE:
			throw BlazingFileSystemException("The per process limit of files has been reached. Consider increasing the per process limit using ulimit");
		case ENOMEM:
			throw BlazingOutOfMemoryException("System ran out of memory");
		case ENOTDIR:
			throw BlazingFileSystemException(uri.toString() + " is not a directory ");
	}
}
...
bool LocalFileSystem::Private::exists(const Uri &uri) const {
	...
	if (success == 0) {
		...
		if (errno != 0 && errno != ENOENT) { // #define ENOENT       2  /* No such file or directory */
			...
			switch (errno) {
				case EACCES:
					throw BlazingInvalidPermissionsFileException(uriWithRoot);
				case EBADF:
					throw BlazingFileSystemException("There was an error checking the existence of " + uriWithRoot.toString());
				case EFAULT:
					throw BlazingInvalidPathException(uriWithRoot);
				case ELOOP:
					throw BlazingFileSystemException("There were too many symbolic links to follow when checking the existence of " + uriWithRoot.toString());
				case ENOMEM:
					throw BlazingOutOfMemoryException("System ran out of memory");
				case ENOTDIR:
					throw BlazingInvalidPathException(uriWithRoot);
			...
			//TODO percy overload stream ops for path and uri std::cout<<"Error checking existence of "<<path<<"  . Error was: "<<errno<<std::endl;
		}
		return false;
	}
}
	...
	if (success == 0) {
		...
		switch (errno) {
			...
			case EFAULT:
				throw BlazingInvalidPathException(uriWithRoot);
			case ELOOP:
				throw BlazingFileSystemException("There were too many symbolic links to follow when getting file status of " + uriWithRoot.toString());
			case ENOMEM:
				throw BlazingOutOfMemoryException("System ran out of memory");
			case ENOTDIR:
				throw BlazingInvalidPathException(uriWithRoot);
			case EOVERFLOW:
				throw BlazingFileSystemException(
					"The file found at " + uriWithRoot.toString() + " is to big to be represented on your platform, are you using a 32 bit operating system?");
		}
	}
	...
	if (dirp) {
		...
		if (this->root.isRoot()) { // if root is '/' then we don't need to replace the uris to relative paths
			...
			while ((entry = readdir(dirp)) != NULL) {
				...
				const FileStatus fullFileStatus(fullUri, relativeFileStatus.getFileType(), relativeFileStatus.getFileSize());
	...
	if (dirp) {
		...
		if (this->root.isRoot()) { // if root is '/' then we don't need to replace the uris to relative paths
			while ((entry = readdir(dirp)) != NULL) {
				...
				const Path fullPath = path + name;
				const bool pass = WildcardFilter::match(fullPath.toString(false), finalWildcard);
...
std::vector<std::string> LocalFileSystem::Private::listResourceNames(const Uri &uri, FileType fileType, const std::string &wildcard) const {
...
std::vector<std::string> LocalFileSystem::Private::listResourceNames(const Uri &uri, const std::string &wildcard) const {
	std::vector<std::string> response;
...
bool LocalFileSystem::Private::makeDirectory(const Uri &uri) const {
	...
	if (!dirExists){
		...
		while (success != 0) {
			...
			if (success == -1 && stat(path.toString().c_str(), &sb) == 0 && S_ISDIR(sb.st_mode)) { // if failed, check again if it exists or not
				success = 0;
				break;
			}
			...
			if (countInvalids % 5 == 0) {
				...
				Logging::Logger().logError("LocalFileSystem::Private::makeDirectory failed to make directory: " + path.toString() + " try number " + std::to_string(countInvalids)
		...
		if (success == 0 && countInvalids > 0) {
			Logging::Logger().logError("LocalFileSystem::Private::makeDirectory finally succeeded making directory: " + path.toString() + "   Succeeded after "
				+ std::to_string(countInvalids) + " tries");
		}
	}
	...
	if (success == -1 && (stat(path.toString().c_str(), &sb) != 0 || !S_ISDIR(sb.st_mode))) { // if failed, check again if it exists or not
		...
		switch (errno) {
			case EACCES:
				throw BlazingInvalidPermissionsFileException(uriWithRoot);
			case EDQUOT:
				throw BlazingFileSystemException("Quota of diskblocks or indoes has been exhausted could not make directory " + uriWithRoot.toString());
			case EEXIST:
				throw BlazingFileSystemException("Pathname " + uriWithRoot.toString() + " already exists.");
			case ENOENT:
				throw BlazingFileNotFoundException("File at " + uriWithRoot.toString() + " does not exist.");
			case EFAULT:
				throw BlazingInvalidPathException(uri);
			case ELOOP:
				throw BlazingFileSystemException("There were too many symbolic links to follow when making directory " + uriWithRoot.toString());
			case EMLINK:
				throw BlazingFileSystemException("There were too many symbolic links to follow when making directory " + uriWithRoot.toString());
			case ENOMEM:
				throw BlazingOutOfMemoryException("System ran out of memory");
			case ENOSPC:
				throw BlazingFileSystemException("FileSystem out of space when making " + uriWithRoot.toString());
			case ENOTDIR:
				throw BlazingInvalidPathException(uri);
			case EPERM:
				throw BlazingFileSystemException("The path " + uriWithRoot.toString() + " cannot be created. That path is immutable.");
			case EROFS:
				throw BlazingFileSystemException("The path " + uriWithRoot.toString() + " cannot be created. This is a read-only file system.");
			case ENAMETOOLONG:
				throw BlazingFileSystemException("path " + uriWithRoot.toString() + " is too long.");
		}
	}
...
bool LocalFileSystem::Private::remove(const Uri &uri) const {
	...
	if (status.isDirectory()) {
		...
		while (success == -1) {
			...
			Logging::Logger().logError("Was unable to remove folder for " + path.toString() + " | Error was: " + sysError + " (" + std::to_string(errno) + ")");
			success = rmdir(path.toString().c_str());
		...
		while (success == -1) {
			...
			Logging::Logger().logError("Was unable to remove file for " + path.toString() + " | Error was: " + sysError + " (" + std::to_string(errno) + ")");
			success = success && std::remove(path.toString().c_str());
...
bool LocalFileSystem::Private::move(const Uri &src, const Uri &dst) const {
	...
	if (!exists(src)) {
		Logging::Logger().logError("Can't rename " + src.toString() + " to " + dst.toString() + ". Could not find the file.");
		return false;
	}
	...
	while (dataFile == -1) {
		...
		const std::string sysError(std::strerror(errno));
		Logging::Logger().logError("Was unable to rename " + srcWithRoot.toString() + " to " + dstWithRoot.toString() + " | Error was: " + sysError + " (" + std::to_string(errno)
			+ ")");
	...
	if (dataFile == -1) {
		Logging::Logger().logError("Can't rename " + srcWithRoot.toString() + " to " + dstWithRoot.toString() + ". Error was: " + std::to_string(errno));

		switch (errno) {
			case EACCES:
				throw BlazingFileSystemException("You do not have permissions to move the file " + srcWithRoot.toString() + " to " + dstWithRoot.toString());
			case ENOTEMPTY:
				throw BlazingFileSystemException("The directory " + dstWithRoot.toString() + " is not empty ");
			case ENOENT:
				throw BlazingFileNotFoundException(srcWithRoot);
			case EINVAL:
				throw BlazingFileSystemException("The path " + srcWithRoot.toString() + " contains " + dstWithRoot.toString());
			case EISDIR:
				throw BlazingFileSystemException(
					"The destination path " + dstWithRoot.toString() + " is a directory but the source " + src.toString() + " is not.");
			case EMLINK:
				throw BlazingFileSystemException(
					"Moving " + srcWithRoot.toString() + " to " + dstWithRoot.toString() + " would result in too many link entries.");
			case ENOSPC:
				throw BlazingFileSystemException(
					"Moving " + srcWithRoot.toString() + " to " + dstWithRoot.toString() + " would require too long a name for the destination path.");
			case EROFS:
				throw BlazingFileSystemException(
					"Moving " + srcWithRoot.toString() + " to " + dstWithRoot.toString() + " would invovle writing to a read only filesystem.");
			case EXDEV:
				throw BlazingFileSystemException(
					"Moving " + srcWithRoot.toString() + " to " + dstWithRoot.toString() + " is not possible because the files are on different filesystems.");
		}
	}
	return (dataFile != -1);
}
...
bool LocalFileSystem::Private::truncateFile(const Uri &uri, long long length) const {
	...
	if (success == -1) {
		...
		switch (errno) {
			case EACCES:
				throw BlazingFileSystemException(
					"The path " + uriWithRoot.toString() + " is either a directory or not a writeable location. Check permissions");
			case EINTR:
				throw BlazingFileSystemException("Truncating " + uriWithRoot.toString() + " was interrupted by a signal");
			case EINVAL:
				throw BlazingFileSystemException(
					"The file " + uriWithRoot.toString() + " was attempting to be truncated with a length of " + std::to_string(length));
			case EIO:
				throw BlazingFileSystemException("A hardware I/O error occurred while truncating " + uriWithRoot.toString());
			case EPERM:
				throw BlazingFileSystemException("The path " + uriWithRoot.toString() + " cannot be truncated. It is append only or immutable.");
			case EFBIG:
				throw BlazingFileSystemException(
					"The file found at " + uriWithRoot.toString() + " is to big to be represented on your platform, are you using a 32 bit operating system?");
		}
