...
RegionResult getRegion(std::string bucketName, Aws::Auth::AWSCredentials credentials) {
	...
	if (locationRequestOutcome.IsSuccess()) {
		const Aws::S3::Model::BucketLocationConstraint regionEnum = locationRequestOutcome.GetResult().GetLocationConstraint();
		regionResult.regionName = Aws::S3::Model::BucketLocationConstraintMapper::GetNameForBucketLocationConstraint(regionEnum);
...
//			std::cout << "Bucket region: " << regionResult.regionName << std::endl;
		...
		const std::string errorPattern =
			"Unable to parse ExceptionName: AuthorizationHeaderMalformed Message: The authorization header is malformed; the region 'us-east-1' is wrong; expecting '";
...
bool S3FileSystem::Private::connect(const FileSystemConnection &fileSystemConnection) {
	...
	const EncryptionType encryptionType = encryptionTypeFromName(fileSystemConnection.getConnectionProperty(ConnectionProperty::ENCRYPTION_TYPE));
	const std::string kmsKeyAmazonResourceName = fileSystemConnection.getConnectionProperty(ConnectionProperty::KMS_KEY_AMAZON_RESOURCE_NAME);
	...
	//TODO percy check valid conn properties: do this in the fs::isValidConn or inside fsConnection (get rid of strings map: that is a bad pattern)
	...
	if (regionResult.valid == false) {
		...
		this->disconnect();
		throw BlazingS3Exception("Error getting region from bucket " + bucketName);
...
//TODO NOTE This code is when we need to support client side encryption (currently only support server side encryption)
...
//			const auto simpleMaterials = Aws::MakeShared<Aws::S3Encryption::Materials::SimpleEncryptionMaterials>("s3Encryption", masterKey);
//			const Aws::S3Encryption::CryptoConfiguration cryptoConfiguration(Aws::S3Encryption::StorageMethod::METADATA, Aws::S3Encryption::CryptoMode::AUTHENTICATED_ENCRYPTION);
//
//			this->s3Client = std::make_shared<Aws::S3Encryption::S3EncryptionClient>(simpleMaterials, cryptoConfiguration, credentials, clientConfig);
//		}
//		break;
//
...
//			}
//
//			auto kmsMaterials = Aws::MakeShared<Aws::S3Encryption::Materials::KMSEncryptionMaterials>("s3Encryption", kmsKeyAmazonResourceName);
//			const Aws::S3Encryption::CryptoConfiguration cryptoConfiguration(Aws::S3Encryption::StorageMethod::INSTRUCTION_FILE, Aws::S3Encryption::CryptoMode::STRICT_AUTHENTICATED_ENCRYPTION);
//
//			this->s3Client = std::make_shared<Aws::S3Encryption::S3EncryptionClient>(kmsMaterials, cryptoConfiguration, credentials, clientConfig);
//		}
//		break;
//
...
bool S3FileSystem::Private::exists(const Uri &uri) const {
	...
	if (result.IsSuccess()) {
		...
		if (objectKey[objectKey.size() - 1] != '/') { // lets see if it was actually a directory and we simply did not have the slash at the end
			objectKey += '/';
...
//				if (shouldRetry){
//					std::cout<<"ERROR: "<<outcome.GetError().GetExceptionName()<<" : "<<outcome.GetError().GetMessage()<<"  SHOULD RETRY"<<std::endl;
//				} else {
//					std::cout<<"ERROR: "<<outcome.GetError().GetExceptionName()<<" : "<<outcome.GetError().GetMessage()<<"  SHOULD NOT RETRY"<<std::endl;
//				}
	...
	if (objectsOutcome.IsSuccess()) {
		if (this->root.isRoot()) { // if root is '/' then we don't need to replace the uris to relative paths
			...
			for (auto const &s3Object : objects) {
				//WARNING TODO percy there is no folders concept in S3 ... we should change Path::isFile::bool to Path::ObjectType::Unkwnow,DIR,FILE,SYMLIN,ETC
			...
			for (auto const &s3Folder : folders) {
				//WARNING TODO percy there is no folders concept in S# ... we should change Path::isFile::bool to Path::ObjectType::Unkwnow,DIR,FILE,SYMLIN,ETC
				const Path path("/" + s3Folder.GetPrefix(), true); //TODO percy avoid hardcoded string
				const Uri entry(uri.getScheme(), uri.getAuthority(), path);
				const FileStatus fileStatus(entry, FileType::DIRECTORY, 0); //TODO percy get info about the size of this kind of object
				const bool pass = filter(fileStatus);
			...
			for (auto const &s3Object : objects) {
				...
				if (fullPath != folderPath) {
					...
					if (pass) {
						...
						const FileStatus relativeFileStatus(relativeUri, fullFileStatus.getFileType(), fullFileStatus.getFileSize());
		...
		if (shouldRetry){
			Logging::Logger().logError(objectsOutcome.GetError().GetExceptionName() + " : " + objectsOutcome.GetError().GetMessage() + "  SHOULD RETRY");
		} else {
			Logging::Logger().logError(objectsOutcome.GetError().GetExceptionName() + " : " + objectsOutcome.GetError().GetMessage() + "  SHOULD NOT RETRY");
		}
		throw BlazingFileSystemException("Could not list files found at " + uriWithRoot.toString() + ". Problem was " + objectsOutcome.GetError().GetExceptionName() + " : " + objectsOutcome.GetError().GetMessage());
	}
	...
	if (objectsOutcome.IsSuccess()) {
		...
		if (this->root.isRoot()) { // if root is '/' then we don't need to replace the uris to relative paths
			...
			for (auto const &s3Object : objects) {
				...
				if (path != folderPath) {
					const bool pass = WildcardFilter::match(path.toString(false), finalWildcard);
					if (pass) {
						const Uri entry(uri.getScheme(), uri.getAuthority(), path);
			...
			for (auto const &s3Object : objects) {
				...
				if (fullPath != folderPath) {
					const bool pass = WildcardFilter::match(fullPath.toString(false), finalWildcard); //filter must use the full path
...
std::vector<std::string> S3FileSystem::Private::listResourceNames(const Uri &uri, FileType fileType, const std::string &wildcard) const {
	...
	if (objectsOutcome.IsSuccess()) {
		...
		if (this->root.isRoot()) { // if root is '/' then we don't need to replace the uris to relative paths
			...
			for (auto const &s3Object : objects) {
				...
				if (fullPath != folderPath) {
					...
					if (pass) {
						response.push_back(fullFileStatus.getUri().getPath().getResourceName()); // resource name is the same for full paths or relative paths
					}
				}
			}
			...
			for (auto const &s3Folder : folders) {
				...
				if (fullPath != folderPath) {
					const Uri fullUri(uri.getScheme(), uri.getAuthority(), fullPath);
					const FileStatus fullFileStatus(fullUri, FileType::DIRECTORY, 0); //TODO percy get info about the size of this kind of object
		...
		throw BlazingFileSystemException("Could not list resources found at " + uriWithRoot.toString() + ". Problem was " + objectsOutcome.GetError().GetExceptionName() + " : " + objectsOutcome.GetError().GetMessage());
	}
	...
	if (objectsOutcome.IsSuccess()) {
		...
		if (this->root.isRoot()) { // if root is '/' then we don't need to replace the uris to relative paths
			...
			for (auto const &s3Object : objects) {
				...
				if (path != folderPath) {
					const Uri entry(uri.getScheme(), uri.getAuthority(), path);
					const bool pass = WildcardFilter::match(uri.toString(false), finalWildcard);
			...
			for (auto const &s3Object : objects) {
				...
				if (fullPath != folderPath) {
					const Uri fullUri(uri.getScheme(), uri.getAuthority(), fullPath);
					const bool pass = WildcardFilter::match(fullUri.toString(false), finalWildcard); //filter must use the full path

					if (pass) {
						response.push_back(fullUri.getPath().getResourceName()); // resource name is the same for full paths or relative paths
					}
				}
			}
...
bool S3FileSystem::Private::makeDirectory(const Uri &uri) const {
	...
	if (result.IsSuccess()) {
		...
		if (shouldRetry){
			Logging::Logger().logError(result.GetError().GetExceptionName() + " : " + result.GetError().GetMessage() + "  SHOULD RETRY");
		} else {
			Logging::Logger().logError(result.GetError().GetExceptionName() + " : " + result.GetError().GetMessage() + "  SHOULD NOT RETRY");
		}
		throw BlazingS3Exception("Could not make directory " + uriWithRoot.toString() + ". Problem was " + result.GetError().GetExceptionName() + " : " + result.GetError().GetMessage());
	}
}
...
bool S3FileSystem::Private::remove(const Uri &uri) const {
	...
	if (result.IsSuccess()) {
		...
		throw BlazingS3Exception("Could not remove " + uriWithRoot.toString() + ". Problem was " + result.GetError().GetExceptionName() + " : " + result.GetError().GetMessage());
...
bool S3FileSystem::Private::move(const Uri &src, const Uri &dst) const {
	...
	if (result.IsSuccess()) {
		...
		Logging::Logger().logError("S3FileSystem::Private::move failed for src URI: " + src.toString() + "  and dst UTI: " + dst.toString());
		bool shouldRetry = result.GetError().ShouldRetry();
		if (shouldRetry){
			Logging::Logger().logError(result.GetError().GetExceptionName() + " : " + result.GetError().GetMessage() + "  SHOULD RETRY");
		} else {
		...
		throw BlazingS3Exception("Could not move " + src.toString() + " to " + dst.toString() + ". Problem was " + result.GetError().GetExceptionName() + " : " + result.GetError().GetMessage());
	...
	if (fileStatus.getFileSize() > length) { // truncate
		...
		if (removeOk == false) {
			throw BlazingS3Exception(
				"Could not remove previous " + uri.toString() + " for truncating (S3 has no truncate so we must copy delete old put in truncated version).");
		}
...
const S3FileSystemConnection::EncryptionType S3FileSystem::Private::encryptionType() const {
	using namespace S3FileSystemConnection;
	const std::string encryptionTypeName = this->fileSystemConnection.getConnectionProperty(ConnectionProperty::ENCRYPTION_TYPE);
...
const Aws::S3::Model::ServerSideEncryption S3FileSystem::Private::serverSideEncryption() const {
	...
	switch (this->encryptionType()) {
		case EncryptionType::AES_256:
			return Aws::S3::Model::ServerSideEncryption::AES256;
			break;
		case EncryptionType::AWS_KMS:
			return Aws::S3::Model::ServerSideEncryption::aws_kms;
			break;
	}
...
const std::string S3FileSystem::Private::getSSEKMSKeyId() const {
	using namespace S3FileSystemConnection;
	const std::string kmsKey = this->fileSystemConnection.getConnectionProperty(ConnectionProperty::KMS_KEY_AMAZON_RESOURCE_NAME);
	return kmsKey;
}
