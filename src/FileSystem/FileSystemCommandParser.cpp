...
FileSystemEntity parseRegisterFileSystem(const std::string &command, std::string &error) {
	...
	const FileSystemType fileSystemType = (isLocal)? FileSystemType::LOCAL : (isHdfs)? FileSystemType::HDFS : FileSystemType::S3;
	...
	const std::string fileSystem = (fileSystemType == FileSystemType::LOCAL)? "local" : Uri::fileSystemTypeToScheme(fileSystemType);
	...
	switch (authSplitTokens.size()) {
		...
		case 2: {
			...
			const std::string connectionArgs = connRoot.substr(openParenthesesToken + 1, (closeParenthesesToken - openParenthesesToken) - 1);
			...
			if (isLocal) {
				...
				const HadoopFileSystemConnection::DriverType driverType = HadoopFileSystemConnection::driverTypeFromName(connectionTokens[3]);
				...
				const S3FileSystemConnection::EncryptionType encryptionType = S3FileSystemConnection::encryptionTypeFromName(connectionTokens[1]);
				...
				fileSystemConnection = FileSystemConnection(bucketName, encryptionType, kmsKeyAmazonResourceName, accessKeyId, secretKey, sessionToken);
			}
