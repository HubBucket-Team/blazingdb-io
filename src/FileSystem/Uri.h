...
class Uri {
	public:
		...
		Uri(FileSystemType fileSystemType, const std::string &authority, const Path &path, bool strict = false); // convenience ctor, will use Uri(scheme, auth, Path, strict)
		Uri(const Uri &other);
		Uri(Uri &&other);
		~Uri();
		...
		bool isParentOf(const Uri &child) const; // this function assumes that child is a folder and child.scheme/auth == this.scheme/auth (false in case invalid or empty)
