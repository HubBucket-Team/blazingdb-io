...
namespace S3FileSystemConnection {
	...
	enum class ConnectionProperty : char {
		...
		KMS_KEY_AMAZON_RESOURCE_NAME, // if encryption type is AWS_KMS, then the user must provide the Amazon Resource Name (ARN) of the AWS KMS key to use
		ACCESS_KEY_ID,
		SECRET_KEY,
		SESSION_TOKEN
	};
...
class FileSystemConnection {
	public:
		...
		*/
		 ...
		 *
		 * @note
		 * Current implementation of this constructor will only accept FileSystemType::LOCAL, any other value will construct an invalid FileSystemConnection
		*/
		...
		*/
		...
		const std::string getConnectionProperty(HadoopFileSystemConnection::ConnectionProperty connectionProperty) const noexcept;

		// is property is not present or the instance is invalid and not S3 then return empty string
		const std::string getConnectionProperty(S3FileSystemConnection::ConnectionProperty connectionProperty) const noexcept;
