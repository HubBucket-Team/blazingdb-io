...
class FileSystemInterface {
	public:
		 ...
		 *
		 * Returns an empty list if the URI is invalid or the directory does not exist or if nothing matches the wildcard pattern.
		 *
		 * @note
		...
		/**
		 * Convenience method, will use list(const Uri &uri, const FileFilter &filter) under the hood in order to do the job.
		 */
		virtual std::vector<FileStatus> list(const Uri &uri, FileType fileType, const std::string &wildcard = "*") const = 0;
		 ...
		 *
		 * @note
		 * This call is faster than other list methods since doesn't need to get the file status info for each file in order to apply the filter.
		 *
		 * @param uri must represent a directory
		 * @param wildcard is the string pattern used to filter the list
		 * @return list of URIs
		 ...
		 */
		virtual std::vector<std::string> listResourceNames(const Uri &uri, FileType fileType, const std::string &wildcard = "*") const = 0;
		 ...
		 *
		 * @note
		 * This call is faster than other listResourceNames methods since doesn't need to get the file status info for each file in order to apply the filter.
		 *
		 * @param uri must represent a directory
		 * @param wildcard is the string pattern used to filter the list
		 * @return list of URI resource names
